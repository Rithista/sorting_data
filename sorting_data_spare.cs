...
namespace sorting_data
{
  ...
  public class Seismic
  {
    ...
    public static void printData()
    {
      ...
      Console.WriteLine("|YEAR      |MONTH          |DAY  |TIME    |MAGNI   |LAT     |LONG      |DEPTH   |REGION                        |IRIS      |TIMESTAMP");
      Console.WriteLine("|__________|_______________|_____|________|________|________|__________|________|______________________________|__________|_________");
      string dataPrintOut;
      List<string[]> dataObjects = new List<string[]>();
      string[] infoArray = new string[11];
      for (int i = 0; i < year.Length; i++)
      {
        dataPrintOut = string.Format("|{0,-10}|{1,-15}|{2,-5}|{3,-8}|{4,-8}|{5,-8}|{6,-10}|{7,-8}|{8,-30}|{9,-10}|{10,-12}", year[i], month[i], day[i], time[i], magnitude[i], latitude[i], longitude[i], depth[i], region[i], iris[i], timestamp[i]);
        Console.WriteLine(dataPrintOut);
        infoArray = new string[11] { year[i], month[i], day[i], time[i], magnitude[i], latitude[i], longitude[i], depth[i], region[i], iris[i], timestamp[i] };
        dataObjects.Add(infoArray);

      }
      dataObjects = dataObjects.OrderBy(arr => arr[0]).ThenBy(arr => arr[1]).ToList();


      string[] sortMonths = new string[] {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December" };
      // Array.Sort(sortMonths, infoArray);


      for (int i = 0; i < dataObjects.Count; i++)
      {
        foreach (var j in dataObjects.ElementAt(i))
        {
            if (j.Contains("2016"))
            {
             Console.Write(j + " ");
